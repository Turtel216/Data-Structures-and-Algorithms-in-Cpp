...
This is an implementation of a MinHeap. The heap has the following methods:
    ...
    - 'extractMin' finds the minimum values, returns it and removes it from the
heap

    - 'MinHeapify' balances the heap recursively

    - 'swap' a helper method to swap 2 values. NOTE the method uses the XOR gate
to swap the values which theoreticly uses less memory, but according to various
sources on the internet it is slower when executing on x86 CPUs. It is left in
to "show off" and because I enjoy trying out diffrent implementations across
these projects
*/
...
public:
  ...
  void decreaseAtIndex(int index, int value) noexcept {
    ...
    if (value >= array[index]) {
      std::cout << "The given value is greater or equal to the current value. "
                   "The given value must be smaller than the current value"
                << std::endl;
      return;
    }
