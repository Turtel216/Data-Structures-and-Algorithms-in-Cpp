...
std::tuple<int, bool> exponetial_search(std::array<int, 10>, int size, int key) noexcept;
...
// Search algorithm to find range for binary_search. Also returns the result of binary_search
std::tuple<int, bool> exponetial_search(std::array<int, 10> arr, int size, int key) noexcept 
{
  // Check if the key is on starting index 
  if (arr[0] == key) return {arr[0], false};
