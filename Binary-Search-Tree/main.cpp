...
public:
  NodeNotFoundException(const char *msg) noexcept : message(msg) {}
...
public:         // both pointers are public for faster development
  ...
  Node(T item) noexcept : value(item) {}
...
private:
  ...
  std::tuple<Node<T> *, Node<T> *> searchParentAndChild(T item) const {
    ...
    while (tempNode != nullptr) {
      // if the node's left child is the target child, return the node and the
      // left child
      if ((tempNode->lChild != nullptr &&
           tempNode->lChild->getValue() == item)) {
        return std::make_tuple(tempNode, tempNode->lChild);
      }

      // if the node's right child is the target child, return the node and the
      // right child
    ...
    throw NodeNotFoundException(
        "The request node or its parent could not be found");
  }
  ...
  void remove(T item) noexcept {
    try {
      ...
      // If the node has a single left child, copy the child and delete the node
        ...
        // if the node is the right child of the parent, delete it and replace
        // it
        else {
          auto deleteme = node;
          node = node->lChild;
      ...
      // If the node has a single right child, copy the child and delete the
      // node
      ...
      // else delete the node with both children and replace it with the inorder
      // successor
      else {
...
int main() {
  ...
  std::cout << "Found node with value: " << tree->search(80)->getValue()
            << std::endl; // TODO should be in try catch

  std::cout << "\nDeleting node with value 70(remove case: 2 children)"
            << std::endl;
  tree->remove(70);
  tree->inorder();

  std::cout << "\nDeleting node with value 60(remove case: 1 child)"
            << std::endl;
  tree->remove(60);
  tree->inorder();

  std::cout << "\nDeleting node with value 20(remove case: 0 children)"
            << std::endl;
  tree->remove(20);
  tree->inorder();
